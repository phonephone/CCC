...
class MyCalorie: UIViewController, ChartViewDelegate {
    ...
    func loadMyCalories() {
        ...
        loadRequest(method:.post, apiName:"my_kcal", authorization:true, showLoadingHUD:firstTime, dismissHUD:true, parameters: parameters){ result in
            ...
            case .success(let responseObject):
                ...
                self.profilePic.sd_setImage(with: URL(string:self.myJSON!["user_img"].stringValue), placeholderImage: UIImage(named: "icon_profile"))
                self.nameLabel.text = self.myJSON!["first_name"].stringValue
                
                self.totalCaloriesLabel.text = self.myJSON!["user_sum_cal"].stringValue
                self.weightLabel.text = self.myJSON!["weight"].stringValue
                self.heightLabel.text = self.myJSON!["height"].stringValue
                self.bmiLabelTop.text = "ดัชนีมวลกาย (BMI) =  \(self.myJSON!["avg_bmi"].stringValue)"
                self.bmiLabelBottom.text = "รูปร่าง = \(self.myJSON!["avg_shape"].stringValue)"
                
                let graphArray = self.myJSON!["data_graph"]
                self.yValues = [
